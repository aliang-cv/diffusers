...
def convert_transformer(state_dict):            # 把模型的字典更换
...
def populate_state_dict(args):
    ...
    converted_state_dict = convert_transformer(original_state_dict)                     # 转换transformer
    model_diffusers = AuraFlowTransformer2DModel(
        num_mmdit_layers=mmdit_layers, num_single_dit_layers=single_dit_layers
    )
    model_diffusers.load_state_dict(converted_state_dict, strict=True)                  # 加载模型
...
if __name__ == "__main__":
    ...
    model_diffusers = populate_state_dict(args)                 # 模型的diffusers
    model_diffusers.save_pretrained(args.dump_path)             # 把模型转成diffusion格式

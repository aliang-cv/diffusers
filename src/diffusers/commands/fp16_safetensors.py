...
class FP16SafetensorsCommand(BaseDiffusersCLICommand):
    ...
    def run(self):
        ...
        model_index = hf_hub_download(repo_id=self.ckpt_id, filename="model_index.json")    # 下载模型
        with open(model_index, "r") as f:
            pipeline_class_name = json.load(f)["_class_name"]                               # 解析模型的类
        pipeline_class = getattr(import_module("diffusers"), pipeline_class_name)           # 获取动态属性
        ...
        pipeline.save_pretrained(                                                           # 这里进行保存模型
            self.local_ckpt_dir,

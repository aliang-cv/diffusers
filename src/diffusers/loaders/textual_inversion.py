...
def load_textual_inversion_state_dicts(pretrained_model_name_or_paths, **kwargs):
    ...
    state_dicts = []
    for pretrained_model_name_or_path in pretrained_model_name_or_paths:            # 遍历预训练模型名称或路径
...
class TextualInversionLoaderMixin:
    ...
    def _maybe_convert_prompt(self, prompt: str, tokenizer: "PreTrainedTokenizer"):  # noqa: F821
        ...
        for token in unique_tokens:
            if token in tokenizer.added_tokens_encoder:
                replacement = token
                i = 1
                while f"{token}_{i}" in tokenizer.added_tokens_encoder:     # 遍历添加的token编码器
    ...
    def load_textual_inversion(
        ...
        **kwargs,
        ...
        # 3. Check inputs
        self._check_text_inv_inputs(tokenizer, text_encoder, pretrained_model_name_or_paths, tokens)    # 检查输入
